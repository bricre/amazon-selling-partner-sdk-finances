 ...
 * Possible values:
 *
 * * Principal - The selling price of the order item, equal to the selling price of
 * the item multiplied by the quantity ordered.
 *
 ...
 *
 * * MarketplaceFacilitatorTax-Other - The tax withheld on other miscellaneous
 * charges.
 *
 ...
 *
 * * RestockingFee - The charge applied to the buyer when returning a product in
 * certain categories.
 *
 * * ReturnShipping - The amount given to the buyer to compensate for shipping the
 * item back in the event we are at fault.
 *
 * * PointsFee - The value of Amazon Points deducted from the refund if the buyer
 * does not have enough Amazon Points to cover the deduction.
 *
 * * GenericDeduction - A generic bad debt deduction.
 *
 * * FreeReplacementReturnShipping - The compensation for return shipping when a
 * buyer receives the wrong item, requests a free replacement, and returns the
 * incorrect item.
 *
 * * PaymentMethodFee - The fee collected for certain payment methods in certain
 * marketplaces.
 *
 * * ExportCharge - The export duty that is charged when an item is shipped to an
 * international destination as part of the Amazon Global program.
 *
 * * SAFE-TReimbursement - The SAFE-T claim amount for the item.
 *
 * * TCS-CGST - Tax Collected at Source (TCS) for Central Goods and Services Tax
 * (CGST).
 *
 * * TCS-SGST - Tax Collected at Source for State Goods and Services Tax (SGST).
 *
 * * TCS-IGST - Tax Collected at Source for Integrated Goods and Services Tax
 * (IGST).
 *
 * * TCS-UTGST - Tax Collected at Source for Union Territories Goods and Services
 * Tax (UTGST).
 */
class ChargeComponent extends AbstractModel
{
    /**
     * The type of charge.
     *
